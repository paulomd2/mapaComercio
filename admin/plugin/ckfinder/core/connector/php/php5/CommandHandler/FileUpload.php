...
class CKFinder_Connector_CommandHandler_FileUpload extends CKFinder_Connector_CommandHandler_CommandHandlerBase {
    ...
    public function sendResponse() {
        ...
        //$sUnsafeFileName = CKFinder_Connector_Utils_FileSystem::convertToFilesystemEncoding(CKFinder_Connector_Utils_Misc::mbBasename(md5(date('Y-m-d H:i:s'.$uploadedFile['name']))));

        $tipoArquivo = pathinfo($uploadedFile['name']);
        $tipoArquivo = '.' . $tipoArquivo['extension'];

        $sFileName= strtolower(md5(date('d/m/Y/H:i:s'))) . $tipoArquivo;
        ...
        while (true) {
            //$sFileName = CKFinder_Connector_Utils_FileSystem::convertToFilesystemEncoding(CKFinder_Connector_Utils_Misc::mbBasename(md5(date('Y-m-d H:i:s'.$uploadedFile['name']))));
